...
import { addCar } from "@/utils/fetchData";
...
const AddCarForm = ({
  ...
  const [addedData, setAddedData] = useState<AddedDataType>({});
  const [warning, setWarning] = useState(false);
  const [imgPreview, setImgPreview] = useState<{ url: string }[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (addedData.images && addedData.images.length > 0) {
      for (let i = 0; i < addedData.images.length; i++) {
        const imgFile = addedData.images[i]?.url;
        if (imgFile) {
    ...
    formData.append("title", addedData.title || "Example");
    formData.append("brand", addedData.brand || "Example");
    formData.append("model", addedData.model || "Example");
    formData.append("year", (addedData.year || 0).toString());
    formData.append("colors", (addedData.colors || 0).toString());
    formData.append("rate", (addedData.rate || 0).toString());
    formData.append("price", (addedData.price || 0).toString());
    formData.append("category", addedData.category || "Example");
    formData.append("mileage", (addedData.mileage || 0).toString());
    formData.append("engine", addedData.engine || "Example");
    formData.append("transmission", addedData.transmission || "Example");
    formData.append("location", addedData.location || "Example");
      ...
      <div className="w-full max-w-6xl bg-white shadow-lg rounded-lg p-8 relative">
        ...
        <form className="" onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className="mt-5">
            ...
            {addedData.images && addedData.images.length > 0 && (
              <span className="ms-2">
                {addedData.images?.length} Images exsist
              </span>
            )}
          </div>
          {imgPop && (
          ...
          {warning && <p className="text-red-600 mt-5">All field required</p>}
